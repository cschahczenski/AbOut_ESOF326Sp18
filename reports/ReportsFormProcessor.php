	...
	else
	{
		...
		if($view == 'overviewOutcome')
		{
			...
			else
			{
				...
				foreach($outcomes as $outcome)
				{
				  for ($i = 0; $i < count($semesters); $i++)//jab fix 2016
				  {
					$colScore[$i] = 0;
					$colStudPass[$i] = 0;
					$colStud[$i] = 0;
					...
					for($i = 0; $i < count($semesters); $i++)
					{
						if($colWithStud[$i] > 0)
						{
							...
							$colAvg[$i][$outIndex] += ($colScore[$i] / $colWithStud[$i]);
							$cols++; //jab moved inside if statement
						}
						//$cols++; //jab moved up inside the if statement
						$pass += $colStudPass[$i];
						$stud += $colStud[$i];
					
						//add semester average for outcome to report
						//$appendThis = sprintf('%.0f%%', $colWithStud[$i] > 0 ?	$colScore[$i]/$colWithStud[$i] : 0);
						if($colWithStud[$i] > 0) //jab - change so if no students, a dash is displayed
						{
							$appendThis = sprintf('%.0f%%', $colScore[$i]/$colWithStud[$i]);
						}
						else
						{
							$appendThis = sprintf('-');
...
/* jab - I think average is incorrect and not very useful
